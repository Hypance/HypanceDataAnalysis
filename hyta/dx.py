...
class DX:
    ...
    def __init__(self,high:pd.Series,low:pd.Series,close:pd.Series,period:int=14) -> None:
        ...
        self.close = close
        self.period = period
        self.dx_data = pd.DataFrame({"High":self.high, "Close":self.close, "Low":self.Low})
        
    def TR(self) -> pd.Series:
        self.dx_data["TR"] = ATR([self.dx_data["High"],self.dx_data["Low"],self.dx_data["Close"]],self.period).true_range()
    ...
    def smoothed_plus_dm14(self):
        ...
        for i in range(len(self.dx_data["+DM 1"])-16):
            ...
            other_14_plus_dm = new_dx_Data[-1] - (new_dx_Data[-1]/self.period) + self.dx_data["+DM 1"][i+16]
    ...
    def smoothed_minus_dm14(self):
        ...
        for i in range(len(self.dx_data["-DM 1"])-16):
            other_14_minus_dm14 = new_dx_Data[-1] - (new_dx_Data[-1]/self.period) + self.dx_data["-DM 1"][i+16]
    ...
    def smoothed_tr14(self):
        ...
        second_14_tr = first_14_tr - (first_14_tr/self.period) + self.dx_data["TR"][self.period+1]
        ...
        for i in range(len(self.dx_data["TR"])- 16):
            other_14_tr = new_tr_Data[-1] - (new_tr_Data[-1]/self.period) + self.dx_data["TR"][i+16]
