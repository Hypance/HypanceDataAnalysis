...
class KAMA:
    ...
    def efficiency_ratio(self):
        self.change()
        self.volatility()
        self.df["ER"] = self.df["Change"] / self.df["Volatility"].rolling(window = self.period).sum()
        return self.df["ER"]

    def smoothing_constant(self):
        self.efficiency_ratio()
        self.df["sc"] = ((self.df["ER"] * (2 / (self.fastest+1) - 2 / (self.slowest+1))) + (2 / (self.slowest+1))) ** 2
        return self.df["sc"]
    ...
    def calc_kama(self):
        ...
        for i in range(1,len(self.df["Close"])):
            ...
            self.df["kama"].iloc[self.period - 1] = self.df["Close"].iloc[self.period - 1]

            if self.df["kama"][i] != np.nan :
                self.df["kama"][i] = self.df["kama"][i-1] + self.df["sc"][i] * (self.df["Close"][i] - self.df["kama"][i-1])              
        return self.df["kama"]