...
class IchimokuCloud:
    ...
    def __init__(self,closes_list:list,lowest_list:list, higest_list:list,period_list:list=[9,26,52,26]):
    ...
    def __tenkan_sen(self) -> float:
        ...
        tenkan_sen = (self.__founder_higest_price(self.ts_period) + self.__founder_lowest_price(self.ts_period))/2
        return tenkan_sen

    
    def __kijun_sen(self) -> float:
        #the formula of Kijun Sen => the sum of the higest and lowest price of last 26 days is divied by 2.
        kijun_sen = (self.__founder_higest_price(self.ks_period) + self.__founder_lowest_price(self.ks_period))/2
        return kijun_sen
    ...
    def __senkou_span_B(self) -> float:
        ...
        senkou_span_B = (self.__founder_higest_price(self.ssb_period) + self.__founder_lowest_price(self.ssb_period))/2
    ...
    def ichimoku_cloud(self) -> pd.Series:
        ...
        functions_name = ["Tenkan Sen","Kijun Sen","Senkou Span A","Senkou Span B","Cikou Span"]
        functions_values = [self.__tenkan_sen(),self.__kijun_sen(),self.__senkou_span_A(),self.__senkou_span_B(),self.__cikou_span()]
