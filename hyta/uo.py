...
class UO:
    ...
    def Average7(self):
        ...
        sevenPeriodedBP = [cumulativeBP[i]-cumulativeBP[i-7] for i in range(7,len(cumulativeBP))]

        cumulativeTR = np.cumsum(self.trueRange())
        cumulativeTR = np.insert(cumulativeTR,0,0)
        
        sevenPeriodedTR = [cumulativeTR[i]-cumulativeTR[i-7] for i in range(7,len(cumulativeTR))]
    ...
    def Average14(self):
        ...
        fourteenPeriodedBP = [cumulativeBP[i]-cumulativeBP[i-14] for i in range(14,len(cumulativeBP))]

        cumulativeTR = np.cumsum(self.trueRange())
        cumulativeTR = np.insert(cumulativeTR,0,0)
        
        fourteenPeriodedTR = [cumulativeTR[i]-cumulativeTR[i-14] for i in range(14,len(cumulativeTR))]
    ...
    def Average28(self):
        ...
        twentyeightPeriodedBP = [cumulativeBP[i]-cumulativeBP[i-28] for i in range(28,len(cumulativeBP))]

        cumulativeTR = np.cumsum(self.trueRange())
        cumulativeTR = np.insert(cumulativeTR,0,0)

        twentyeightPeriodedTR = [cumulativeTR[i]-cumulativeTR[i-28] for i in range(28,len(cumulativeTR))]
