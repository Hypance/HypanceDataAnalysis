...
class MACD:
    ...
    def Ema_1(self) -> list:
        ...
        #Other values of Ema_12 comes from the below calhttp://localhost:8888/notebooks/MACD.ipynb#culation. Ema_12 values listed in "for" loop.
        for i in range(len(self.close)-self.EMA1):
            Ema_first.append(self.close[i+self.EMA1]*(2/(self.EMA1+1))+(Ema_first[i]*(1-(2/(self.EMA1+1)))))
    ...
    def Ema_2(self) -> list:
        ...
        for i in range(len(self.close)-self.EMA2):
            Ema_second.append(self.close[i+self.EMA2]*(2/(self.EMA2+1))+(Ema_second[i]*(1-(2/(self.EMA2+1)))))
    ...
    def Signal(self) -> list:
        ...
        for i in range(len(self.close)-self.EMA2-self.SIGNAL+1):
            signal.append(self.MovingAverageConvergenceDivergence()[i+self.SIGNAL]*(2/(self.SIGNAL+1))+(signal[i]*(1-((2/(self.SIGNAL+1))))))
    ...
    def Histogram(self) -> list:
        ...
        for i in range(len(self.close)-self.EMA2-self.SIGNAL+2):
            histogram.append(self.MovingAverageConvergenceDivergence()[self.SIGNAL-1+i]-self.Signal()[i])
        return histogram
    
    def Result(self) -> pd.Series:
        
        d = {
        "Ema_first": pd.Series(self.Ema_1(),index=[i for i in range(0,len(self.close)-self.EMA1+1)]),
        "Ema_second": pd.Series(self.Ema_2(),index=[i for i in range(self.EMA2-self.EMA1,len(self.close)-self.EMA1+1)]),
        "Histogram": pd.Series(self.Histogram(),index=[i for i in range(self.EMA2-self.EMA1+self.SIGNAL-1,len(self.close)-self.EMA1+1)])
        }
        return d