...
class ParabolicSAR:
    ...
    def parabolic_sar(self) -> list:
        ...
        for i in range(len(high)-1):
            ...
            if trend[i]=="Falling":
                try:
                    initial_psar.append(max((psar[i]-calc_psar[i]),np.array(high[i-1:i+1]).max()))
                except ValueError:
                    initial_psar.append(max((psar[i]-calc_psar[i]),np.array(high[0:1]).max()))
            elif trend[i]=="Rising":
                try:
                    initial_psar.append(min((psar[i]-calc_psar[i]),np.array(low[i-1:i+1]).min()))
                except ValueError:
                    initial_psar.append(max((psar[i]-calc_psar[i]),np.array(low[0:1]).min()))
                    
            #setting psar values according to trend,initial_psar and given data 
            if trend[i]=="Falling" and high[i+1]<initial_psar[i]:
                psar.append(initial_psar[i])
            elif trend[i]=="Falling" and high[i+1]>=initial_psar[i]:
                psar.append(ep[i])
            elif trend[i]=="Rising" and low[i+1]>initial_psar[i]:
                psar.append(initial_psar[i])
            elif trend[i]=="Rising" and low[i+1]<=initial_psar[i]:
                psar.append(ep[i])
            ...
            if trend[i+1]=="Falling":
                ep.append(min(ep[i],low[i+1]))
            else:
                ep.append(max(ep[i],high[i+1]))
