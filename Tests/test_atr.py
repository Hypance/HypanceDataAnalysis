...
class TestAverageTrueRange(unittest.TestCase):
    ...
    def test_tr(self):
        ...
        self.assertEqual(
            round(self.result_tr[-1], 2), round(self.result.true_range()[-1], 2)
        )
    ...
    def test_atr(self):
        ...
        self.true_value1 = 14
        self.assertEqual(
            round(self.result_atr[self.true_value1 + 1], 2),
            round(self.result.average_true_range()[2], 2),
        )
        self.assertEqual(
            round(self.result_atr[15], 2), round(self.result.average_true_range()[2], 2)
        )
        self.assertEqual(self.result_atr[-1], self.result.average_true_range()[-1])
        self.assertEqual(
            len(self.result_atr) - self.true_value1 + 1,
            len(self.result.average_true_range()),
        )
    ...
    def test_tr_atr(self):
        ...
        self.true_value1 = 14
        self.assertGreater(
            len(self.result.true_range()), len(self.result.average_true_range())
        )
        self.assertEqual(
            len(self.result.true_range()) - len(self.result.average_true_range()),
            self.true_value1 - 1,
    ...
    def test_tr_atr_true_value_20(self):
        ...
        self.true_value2 = 20
        self.assertEqual(
            len(self.result_tr_20) - len(self.result_atr_20), self.true_value2 - 1
        )
