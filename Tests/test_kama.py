...
class TestKAMA(unittest.TestCase):
    ...
    def test_change(self):
        pd.testing.assert_series_equal(
            self.df["change"], self.ind.change(), check_names=False
        )
    ...
    def test_volatility(self):
        pd.testing.assert_series_equal(
            self.df["volatility"], self.ind.volatility(), check_names=False
        )
    ...
    def test_smoothing_constant(self):
        self.assertAlmostEqual(
            self.df["SC"].any(), self.ind.smoothing_constant().any(), 4
        )
    ...
    def test_kama(self):
        pd.testing.assert_series_equal(
            self.df["KAMA"], self.ind.calc_kama(), check_names=False
        )
