...
class TestBollinger(unittest.TestCase):
    ...
    def test_bollinger(self):
        pd.testing.assert_series_equal(
            self.df["Upper"], self.indicator.bands()[0], check_names=False
        )
        pd.testing.assert_series_equal(
            self.df["Lower"], self.indicator.bands()[1], check_names=False
        )

        # Checks if the upper band is greater than the lower band
        self.assertGreaterEqual(
            self.indicator.bands()[0][25], self.indicator.bands()[1][25]
        )
        self.assertGreaterEqual(
            self.indicator.bands()[0][24], self.indicator.bands()[1][24]
        )
